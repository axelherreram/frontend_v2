...
const DropdownUser = () => {
    ...
    role === "1"
      ...
      : role === "2"
        ...
        : role === "3"
          ...
          : role === "4"
            ? "/coordinadorsede/perfil"
            : role === "5"
              ? "/coordinadorgeneral/perfil"
              : role === "6"
                ? "/coordinadortesis/perfil"
                : role === "7"
                  ? "/revisortesis/perfil"
  ...
  const settingsLink =
    role === "1"
      ? "/estudiantes/contraseña"
      : role === "2"
        ? "/catedratico/contraseña"
        : role === "3"
          ? "/administrador/contraseña"
          : role === "4"
            ? "/coordinadorsede/contraseña"
            : role === "5"
              ? "/coordinadorgeneral/contraseña"
              : role === "6"
                ? "/coordinadortesis/contraseña"
                : role === "7"
                  ? "/revisortesis/contraseña"
  ...
  return (
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      <Link
        ...
        className="flex items-center gap-4 p-2 rounded-2xl hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 dark:hover:from-gray-700/50 dark:hover:to-gray-600/50 transition-all duration-300"
        to="#"
      ...
      >
        ...
        <span className="hidden text-right lg:block">
          ...
          <span className="block text-xs text-gray-500 dark:text-gray-400 font-medium">{roleName}</span>
        </span>
        {/* Displays the profile photo or user's initial */}
        <span className="h-12 w-12 rounded-full flex items-center justify-center bg-gradient-to-br from-blue-500 to-purple-600 text-white shadow-lg ring-2 ring-white/20 backdrop-blur-sm">
          {profilePhoto ? (
            <img
            ...
            <span className="text-lg font-bold">{userName ? userName.charAt(0).toUpperCase() : ""}</span>
          )}
        </span>
        {/* Arrow icon */}
        <svg
          className={`hidden fill-current sm:block w-3 h-3 text-gray-600 dark:text-gray-400 transition-transform duration-300 ${dropdownOpen ? "rotate-180" : ""}`}
          viewBox="0 0 12 8"
          fill="none"
      ...
      {dropdownOpen && (
        <div
          className={`absolute right-0 mt-4 flex w-64 flex-col rounded-3xl border border-white/20 bg-white/95 backdrop-blur-xl shadow-2xl dark:border-gray-700/50 dark:bg-gray-800/95 animate-in slide-in-from-top-2 duration-300`}
        >
          <ul className="flex flex-col gap-2 border-b border-gray-200/50 dark:border-gray-700/50 px-6 py-6">
            <li>
              <Link
                to={profileLink}
